function [matching_omega_gamma_list, loop_mean_attenuation_list] = plotAttenuation(data, gamma_value, mean_diameter, plot_flag)
    % PLOTATTENUATION Creates attenuation plots for granular media
    % 
    % Inputs:
    ...
    %   loop_mean_attenuation_list - List of attenuation/omega values
    ...
    for p = 1:length(pressure_list)
        ...
        % Initialize attenuation list for this pressure
        loop_mean_attenuation_list = zeros(size(unique_omega_gamma));
        
        % For each omega_gamma value, compute mean attenuation
        for og = 1:length(unique_omega_gamma)
            ...
            % Compute mean alphaoveromega for all matching seeds
            alphaoveromega_values = zeros(length(matching_og_indices), 1);
            for i = 1:length(matching_og_indices)
                idx = matching_og_indices(i);
                % Check if alphaoveromega_x is a cell array or a regular array
                if iscell(data.alphaoveromega_x)
                    alphaoveromega_values(i) = data.alphaoveromega_x{idx};
                else
                    alphaoveromega_values(i) = data.alphaoveromega_x(idx);
                end
            end
            ...
            loop_mean_attenuation_list(og) = mean_diameter * mean(alphaoveromega_values);
        end
        ...
        if plot_flag
            ...
            set(gca, 'YScale', 'log');
            plot(matching_omega_gamma_list, loop_mean_attenuation_list, '-o', ...
