function packBi2dRepXYIso(N, K, D, G, M, P_target, seed, plotit, x_mult, y_mult, calc_eig, save_path)
...
% packBi2dRepXYIso(400,100, 1, 1.4, 1, .05, 1, false, 1, 1, false, 'in/2d_isotropic_tile_20by20/tiles/')
...
% y_mult = 2;
% calc_eig = false;
% save_path = 'in/2d_isotropic_tile_20by20/';

%% Set up section
rng(seed)

Lx = 2*sqrt(N)*D; % box width
Ly = 2*sqrt(N)*D; %starting box height
...
% plotit = 1;  % plot ?
plot_KE = 0;
Nplotskip = 1000;  % number of timesteps to skip before plotting
Ncellupdate = 1000;
...
%% Initial Conditions
[x, y] = ndgrid(D/2:G*D:Lx-D/2,D/2:G*D:Ly-D/2);
...
num_cells_x = (Lx/raw_cell_width);
num_cells_x = round(num_cells_x);
cell_width_x = Lx/num_cells_x;
num_cells_y = (Ly/raw_cell_width);
num_cells_y = round(num_cells_y);
cell_width_y = Ly/num_cells_y;

jj_max = ceil(Lx/cell_width_x);
kk_max = ceil(Ly/cell_width_y);

cell_num_x = ceil(x/cell_width_x);
cell_num_y = ceil(y/cell_width_y);
for jj = 1:jj_max
    for kk = 1:kk_max
        cell_list{jj,kk} = find(cell_num_x == jj & cell_num_y == kk);
    end
end
...
for nt = 1:Nt
    ...
    for jj = 1:jj_max
        for kk = 1:kk_max
            ...
            jjl = mod(jj-2,jj_max)+1;
            jjr = mod(jj,jj_max)+1;
            kkl = mod(kk-2,kk_max)+1;
            kkr = mod(kk,kk_max)+1;
            mm_list = [cell_list{jjl,kkl},cell_list{jjl,kk},cell_list{jjl,kkr},...
                cell_list{jj,kkl},cell_list{jj,kk},cell_list{jj,kkr},...
                cell_list{jjr,kkl},cell_list{jjr,kk},cell_list{jjr,kkr}];

            for nn = cell_list{jj,kk}
                ...
                for mm = mm_list_nn
                    ...
                    if(abs(dy) <= Dnm)
                        ...
                        if(dnm < Dnm^2)
                            ...
                            Fx(nn) = Fx(nn)+F.*dx-Fdiss.*dx/dnm - B*vx(nn);  % particle-particle Force Law
                            Fy(nn) = Fy(nn)+F.*dy-Fdiss.*dy/dnm - B*vy(nn);
                            Zn(nn) = Zn(nn) + 1;
    ...
    if fast_compress_flag
        ...
        elseif (P<P_target && Ek(nt)<1e-8)
            ...
            flag2 = true;
            nt_compress = nt;
        elseif (P>P_target && Ek(nt)<1e-10 && nt > (nt_compress+100))
            Lx = Lx * (1+r_fast);
            x = x.*(1+r_fast);
            Ly = Ly * (1+r_fast);
            y = y.*(1+r_fast);
...
if calc_eig == true
    ...
    filename = [save_path, '2D_N' num2str(N) '_P' num2str(P_target) '_' num2str(Lx), 'by' num2str(Ly) '_Seed' num2str(seed) '.mat'];
