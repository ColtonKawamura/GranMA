...
for i = 1:length(damping_constants)
    ...
    for j = 1:length(pressures)
        ...
        scatter(abs(imag(eigen_values)), -real(eigen_values), damping_constant*100, 'DisplayName', sprintf('P=%.3f, %s', pressure, titles{i}))
        end
    end
    ...
    grid on
    legend
    title('Damping vs Frequency for Different Damping Constants and Pressures')
    hold off
