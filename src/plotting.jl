...
function getMeanField(filtered_data; plot = true)
    ...
    if plot==true
        mat"""
        figure
        scatter($(x_parra), $(prime_field), "*", "DisplayName", "x prime field")
        hold on
        set(gca, 'YScale', 'log')
        grid on
    ...
    if plot==true
        mat"""
        scatter($(x_parra), $(y), "o", "DisplayName", "raw x-amplitude")
        hold on
        set(gca, 'YScale', 'log')
        grid on
        xlabel("\$ x \$", "Interpreter", 'latex', "FontSize", 15)
    ...
    if plot==true
        mat"""
        scatter($(x_parra), $(mean_field), "v","DisplayName", "mean field")
        hold on
        set(gca, 'YScale', 'log')
        grid on
        xlabel("\$ x \$", "Interpreter", 'latex', "FontSize", 15)
    ...
    if plot == true
        mat"""
        scatter($(x_perp), $(y), "o", "DisplayName", "raw y-amplitude")
        set(gca, 'YScale', 'log')
        grid on
    ...
    mean_field = -(omega/wavespeed).*distance_from_wall
    mean_field = mod.(mean_field, 2π)
    prime_field = phase .- mean_field
    prime_field = mod.(prime_field, 2π)


    if plot==true
        mat"""
        figure
        scatter($(distance_from_wall), $(prime_field), "*", "DisplayName", "x prime field")
        hold on
        grid on
        xlabel("\$ x \$", "Interpreter", 'latex', "FontSize", 15)
    ...
    if plot==true
        mat"""
        scatter($(distance_from_wall), $(phase), "o", "DisplayName", "raw x-phase")
        hold on
        grid on
        xlabel("\$ x \$", "Interpreter", 'latex', "FontSize", 15)
        ylabel("\$\\phi(x \$", "Interpreter", 'latex', "FontSize", 15)
    ...
    if plot==true
        mat"""
        scatter($(distance_from_wall), $(mean_field), "v","DisplayName", "mean field")
        grid on
        xlabel("\$ x \$", "Interpreter", 'latex', "FontSize", 15)
        ylabel("\$ \\phi(x) \$", "Interpreter", 'latex', "FontSize", 15)
        set(get(gca, 'ylabel'), 'rotation', 0);
    ...
    if plot == true
        mat"""
        scatter($(x_perp), $(y), "o", "DisplayName", "raw y-phase")
        grid on
        legend('show', 'Location', 'northeast', 'Interpreter', 'latex');
        set(gca, 'FontSize', 15)
        legend('FontSize', 15)
