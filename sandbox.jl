...
function plot_ωγ_attenuation_2d(gamma_value)  # Need to fix the legend
...
function plot_ellipse_width_effect(ω_value, γ_value)
    ...
    for width in width_list
        ...
        simulation_data = load("out/processed/2d_bi_K100_W$(width).jld2")
        plot_ellipse_pdf(ω_value, γ_value) # Need to fix the pressure normalization; also need to output the data for another plot
        # plot that data and hold on the figure
    end


    # loop through length of number of widths each width
        # load the struct
        # plot the curve without a pressure normilzation
    # 
    # filter the data based on those that are close to gamma_value
    closest_γ_index = argmin(abs.([idx.gamma for idx in simulation_data] .- γ_value))
    closest_γ_value = simulation_data[closest_γ_index].gamma
